...
class NoteService {
    ...
    suspend fun delete(context: Context, space: Space, note: SpaceNote) {
        ...
        note.attachments.forEach { runCatching { awaitStorageDelete(it.storagePath) } }
    ...
    private suspend fun awaitUpload(path: String, bytes: ByteArray) = suspendCancellableCoroutine<Unit> { c -> storage.reference.child(path).putBytes(bytes).addOnSuccessListener { c.resume(Unit) {} }.addOnFailureListener(c::resumeWithException) }
