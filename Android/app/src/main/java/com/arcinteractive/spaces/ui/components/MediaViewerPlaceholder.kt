...
fun MediaViewerPlaceholder(
    ...
    var gifPath by remember(media.id) { mutableStateOf<String?>(null) }
    ...
    LaunchedEffect(media.id) {
        runCatching {
            ...
            } else if (media.type == MessageType.Gif || media.mediaCategory?.equals("gif", ignoreCase = true) == true) {
                val bytes = encryptedMediaService.loadFullMediaBytes(context, media)
                val file = encryptedMediaService.writeTempMediaFile(
                    context = context,
                    bytes = bytes,
    ...
    Dialog(
        ...
        Box(
            ...
            when {
                ...
                gifPath != null -> {
                    AnimatedGifViewer(filePath = gifPath!!)
                }
                            ...
                            if (bytes == null) {
                                ...
                                saveScope.launch {
                                    val result = if (media.type == MessageType.Video) {
                                        ...
                                        if (media.type == MessageType.Gif || media.mediaCategory?.equals("gif", ignoreCase = true) == true) {
...
@Composable
private fun AnimatedGifViewer(filePath: String) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        ...
        AndroidView(
            ...
            update = { webView ->
                val html = """
                    <html>
                    <body style="margin:0;padding:0;background:black;overflow:hidden;display:flex;align-items:center;justify-content:center;width:100%;height:100%;">
                        <img src="${file.name}" style="max-width:100%;max-height:100%;object-fit:contain;pointer-events:none;user-select:none;-webkit-user-select:none;" />
                    </body>
                    </html>
                """.trimIndent()
