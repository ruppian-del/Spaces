...
import com.arcinteractive.spaces.data.spaces.SpaceNotificationCategory
import com.arcinteractive.spaces.data.spaces.SpaceNotificationPreference
import com.arcinteractive.spaces.data.spaces.SpaceNotificationPreferencesService
...
fun YouScreen(
    ...
    Scaffold(
        ...
        if (activeDetail == null) {
            LazyColumn(
                ...
                item {
                    SettingsSectionCard(title = "Notifications") {
                        ...
                        SettingsNavigationRow("Space Notifications") {
                            activeDetail = YouDetailDestination.SpaceNotifications
                        }
            ...
            if (activeDetail == YouDetailDestination.SpaceNotifications) {
...
private fun DetailContent(
    ...
    when (destination) {
        YouDetailDestination.SpaceNotifications -> Box(modifier = modifier)
...
@Composable
private fun SpaceNotificationsContent(modifier: Modifier) {
    val context = LocalContext.current
    val scope = rememberCoroutineScope()
    val spaceService = remember { SpaceService() }
    val preferenceService = remember { SpaceNotificationPreferencesService() }
    var spaces by remember { mutableStateOf<List<Space>>(emptyList()) }
    var preferences by remember { mutableStateOf<Map<String, SpaceNotificationPreference>>(emptyMap()) }
    var selectedSpaceId by rememberSaveable { mutableStateOf<String?>(null) }
    var saving by remember { mutableStateOf(false) }
    var errorMessage by remember { mutableStateOf<String?>(null) }

    DisposableEffect(Unit) {
        val listener = spaceService.listenToSpacesForCurrentUser(
            context = context,
            listenerKey = "you.space-notification-settings"
        ) { result ->
            result.onSuccess { spaces = it.sortedBy { space -> space.name.lowercase() } }
                .onFailure { errorMessage = it.localizedMessage }
        }
        onDispose { listener?.remove() }
    }
    LaunchedEffect(Unit) {
        runCatching { preferenceService.load(context) }
    ...
    if (selectedSpace == null) {
        LazyColumn(
            ...
            item {
                Text(
                    "Choose a Space to customize which updates notify you. Activity remains available when notifications are disabled.",
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            }
            if (spaces.isEmpty()) {
                item { EmptySettingsText("No Spaces available.") }
            }
            items(spaces, key = { it.id }) { space ->
                val enabled = preferences[space.id]?.allEnabled ?: true
                Surface(
                    modifier = Modifier
                        .fillMaxWidth()
                    ...
                    Row(
                        ...
                        Text(space.emoji, fontSize = 24.sp)
                        Text(space.name, modifier = Modifier.weight(1f), fontWeight = FontWeight.SemiBold)
                        Text(if (enabled) "On" else "Off", color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        val preference = preferences[selectedSpace.id] ?: SpaceNotificationPreference()
        val moduleOptions = listOf(
            Triple(SpaceNotificationCategory.Announcements, "Announcements", "Important Space updates"),
            Triple(SpaceNotificationCategory.Rooms, "Room Messages", "New messages and attachments"),
            Triple(SpaceNotificationCategory.Notes, "Notes", "New Notes"),
            Triple(SpaceNotificationCategory.Lists, "Lists", "New shared Lists"),
            Triple(SpaceNotificationCategory.Events, "Events", "New and updated Events"),
            Triple(SpaceNotificationCategory.Polls, "Polls", "New Polls"),
            Triple(SpaceNotificationCategory.MediaAndFiles, "Media & Files", "New shared content")
        )
        val interactionOptions = listOf(
            Triple(SpaceNotificationCategory.CommentsAndReplies, "Comments & Replies", "Discussion on your content"),
            Triple(SpaceNotificationCategory.Mentions, "Mentions", "When someone mentions you"),
            Triple(SpaceNotificationCategory.Assignments, "List Assignments", "When a List item is assigned to you")
        )
        ...
        LazyColumn(
            ...
            item {
                TextButton(onClick = { selectedSpaceId = null }) { Text("‹ All Spaces") }
            }
            item {
                Text("${selectedSpace.emoji} ${selectedSpace.name}", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
            }
            item {
                SettingsSectionCard(title = "Notifications") {
                    SettingsToggleRow(
                        title = "All Notifications",
                        checked = preference.allEnabled,
                        enabled = !saving,
                        onCheckedChange = { enabled ->
                            val updated = preference.copy(allEnabled = enabled)
                            preferences = preferences + (selectedSpace.id to updated)
                            saving = true
                            scope.launch {
                                runCatching { preferenceService.save(context, selectedSpace.id, updated) }
                                    .onFailure {
                                        preferences = preferences + (selectedSpace.id to preference)
                    ...
                    EmptySettingsText("Turning this off silences this Space without changing your individual choices.")
                }
            }
            item {
                SpaceNotificationOptionsCard(
                    title = "Modules",
                    options = moduleOptions,
                    preference = preference,
                    enabled = preference.allEnabled && !saving,
                    onChange = { category, enabled ->
                        val updated = preference.copy(categories = preference.categories + (category to enabled))
                        preferences = preferences + (selectedSpace.id to updated)
                        saving = true
                        scope.launch {
                            runCatching { preferenceService.save(context, selectedSpace.id, updated) }
...
@Composable
private fun SpaceNotificationOptionsCard(
    title: String,
        ...
        options.forEachIndexed { index, option ->
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(option.third, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                Switch(
                    checked = preference.categories[option.first] ?: true,
                    onCheckedChange = { onChange(option.first, it) },
                    enabled = enabled
                )
            }
...
private fun activeDetailTitle(destination: YouDetailDestination?): String {
    return when (destination) {
        YouDetailDestination.SpaceNotifications -> "Space Notifications"
