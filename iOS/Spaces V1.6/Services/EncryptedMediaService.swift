...
final class EncryptedMediaService {
    ...
    func uploadImage(
        ...
        try requireUploadSize(originalData)
        let organizationID = try await OrganizationService().reserveStorage(forSpaceID: spaceID, bytes: Int64(originalData.count))
        var completed = false
        defer { if !completed { Task { @MainActor in await OrganizationService().releaseStorage(organizationID: organizationID, bytes: Int64(originalData.count)) } } }
        ...
        let result = try await uploadRasterMedia(
            spaceID: spaceID,
            mediaID: mediaID,
            ...
            mediaType: mediaType,
            mimeType: mimeType,
            chargedBytes: originalData.count,
    ...
    private func requireUploadSize(_ data: Data) throws {
        guard data.count <= 1_024 * 1_024 * 1_024 else { throw EncryptedMediaServiceError.uploadTooLarge }
    }
    ...
    func deleteStorageObjects(for metadata: EncryptedMediaMetadata) async throws {
        ...
        let reference = storage.reference(withPath: metadata.storagePath)
        let storedMetadata = try? await reference.getMetadata()
        try await deleteObject(at: reference)
        ...
        let charged = Int64(storedMetadata?.customMetadata?["organizationChargedBytes"] ?? "") ?? Int64(metadata.fileSize)
        if let spaceID = metadata.storagePath.split(separator: "/").dropFirst().first.map(String.init) {
            await OrganizationService().releaseStorage(forSpaceID: spaceID, bytes: charged)
        }
    }
...
enum EncryptedMediaServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .uploadTooLarge:
            return "Each upload is limited to 1 GB."
        }
    }
}
