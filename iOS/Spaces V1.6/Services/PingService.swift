...
final class PingService {
        ...
        return firestore.collection("pings")
            ...
            .addSnapshotListener { snapshot, error in
                ...
                Task { @MainActor [weak self] in
                    guard let self else { return }
                    let reactions = await self.mapReactions(
                        documents: documents,
                        ping: ping,
    ...
    func toggleReaction(_ emoji: String, in ping: Ping, messageID: String) async throws {
        ...
        if currentEmoji == emoji {
            ...
            try await setData([
                "emoji": emoji,
                "userId": session.uid,
                "userName": profile?.displayName ?? session.displayName,
    ...
    private func mapReactions(
        ...
        var reactionRecords: [(emoji: String, userID: String, userName: String?)] = []
        ...
        for document in documents {
            ...
            let userName = (data["userName"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty
            reactionRecords.append((emoji: emoji, userID: userID, userName: userName))
            if userID == currentUserID {
                selectedEmoji = emoji
            }
        }

        var fallbackNames = Dictionary(
            uniqueKeysWithValues: zip(ping.participantIds, ping.participantNames)
                .filter { !$0.1.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        )
        let missingUserIDs = Set(reactionRecords.compactMap {
            $0.userName == nil && fallbackNames[$0.userID] == nil ? $0.userID : nil
        })
        for userID in missingUserIDs {
            if let profile = try? await userProfileService.fetchUserProfile(uid: userID) {
                fallbackNames[userID] = profile.displayName
            }
        }

        for reaction in reactionRecords {
            guard let userName = reaction.userName ?? fallbackNames[reaction.userID],
                  !(namesByEmoji[reaction.emoji] ?? []).contains(userName) else { continue }
            namesByEmoji[reaction.emoji, default: []].append(userName)
        }
