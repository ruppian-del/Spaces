...
final class SpaceService {
    ...
    func listenToSpacesForCurrentUser(onUpdate: @escaping (Result<[Space], Error>) -> Void) -> ListenerRegistration? {
        ...
        let registration = CompositeSpaceListener()
        var directSpaces: [Space] = []
        var organizationSpaces: [String: [Space]] = [:]
        func publish() {
            let merged = (directSpaces + organizationSpaces.values.flatMap { $0 })
                .reduce(into: [String: Space]()) { $0[$1.id] = $1 }
            onUpdate(.success(Array(merged.values)))
        }
        registration.add(firestore.collection("spaces").whereField("memberIds", arrayContains: session.uid).addSnapshotListener { snapshot, error in
            if let error { onUpdate(.failure(error)); return }
            directSpaces = snapshot?.documents.compactMap(self.mapSpace(document:)) ?? []
            publish()
        })
        registration.add(firestore.collection("organizations").whereField("memberIds", arrayContains: session.uid).addSnapshotListener { snapshot, error in
            if let error { onUpdate(.failure(error)); return }
            registration.removeOrganizationListeners()
            organizationSpaces = [:]
            for organization in snapshot?.documents ?? [] {
                organization.reference.collection("members").document(session.uid).getDocument { member, memberError in
                    if let memberError { onUpdate(.failure(memberError)); return }
                    guard ["primary_admin", "admin"].contains(member?.get("role") as? String ?? "") else { publish(); return }
                    let listener = firestore.collection("spaces").whereField("organizationId", isEqualTo: organization.documentID).addSnapshotListener { spacesSnapshot, spacesError in
                        if let spacesError { onUpdate(.failure(spacesError)); return }
                        organizationSpaces[organization.documentID] = spacesSnapshot?.documents.compactMap(self.mapSpace(document:)) ?? []
                        publish()
                    }
                    registration.addOrganization(listener)
                }
            }
            publish()
        })
    ...
    func createSpace(
        ...
        if let organizationID { try await OrganizationService().requireActiveSpaceCapacity(for: organizationID) }
        ...
        if let organizationID {
            for module in resolvedEnabledModules {
                try await OrganizationService().requireModuleEntitlement(for: organizationID, module: module)
            }
        }
        ...
        if let organizationID {
            spaceData["organizationId"] = organizationID
        }
        try await setData(spaceData, for: spaceReference)
        try await setData([
            "userId": session.uid,
    ...
    func redeemInvite(code: String) async throws -> Space {
        ...
        let inviteSnapshot = try await getDocument(inviteReference)
        let inviteData = try validatedInviteData(from: inviteSnapshot)
        if let spaceID = inviteData["spaceId"] as? String {
            let spaceSnapshot = try await getDocument(firestore.collection("spaces").document(spaceID))
            if let organizationID = spaceSnapshot.data()?["organizationId"] as? String {
                try await OrganizationService().requireUniqueMemberCapacity(for: organizationID, admitting: session.uid)
            }
        }
    ...
    func setFilesEnabled(in space: Space, isEnabled: Bool) async throws {
        ...
        if isEnabled, let organizationID = space.organizationId { try await OrganizationService().requireModuleEntitlement(for: organizationID, module: .files) }
    ...
    func setAnnouncementsEnabled(in space: Space, isEnabled: Bool) async throws {
        ...
        try await requirePermission(.manageModules, in: space, error: .invitePermissionDenied)
        if isEnabled, let organizationID = space.organizationId { try await OrganizationService().requireModuleEntitlement(for: organizationID, module: .announcements) }
    ...
    func setRoomsEnabled(in space: Space, isEnabled: Bool) async throws {
        guard let firestore else { throw SpaceServiceError.firestoreNotConfigured }
        try await requirePermission(.manageModules, in: space, error: .invitePermissionDenied)
        if isEnabled, let organizationID = space.organizationId { try await OrganizationService().requireModuleEntitlement(for: organizationID, module: .rooms) }
    ...
    func setListsEnabled(in space: Space, isEnabled: Bool) async throws {
        guard let firestore else { throw SpaceServiceError.firestoreNotConfigured }
        try await requirePermission(.manageModules, in: space, error: .invitePermissionDenied)
        if isEnabled, let organizationID = space.organizationId { try await OrganizationService().requireModuleEntitlement(for: organizationID, module: .lists) }
        var modules = try await latestEnabledModules(in: space)
        if isEnabled {
    ...
    func setNotesEnabled(in space: Space, isEnabled: Bool) async throws {
        guard let firestore else { throw SpaceServiceError.firestoreNotConfigured }
        try await requirePermission(.manageModules, in: space, error: .invitePermissionDenied)
        if isEnabled, let organizationID = space.organizationId { try await OrganizationService().requireModuleEntitlement(for: organizationID, module: .notes) }
        var modules = try await latestEnabledModules(in: space)
    ...
    func addToOrganization(_ organizationID: String, in space: Space) async throws {
        guard let firestore else { throw SpaceServiceError.firestoreNotConfigured }
        guard authService.currentSession() != nil else { throw SpaceServiceError.userNotSignedIn }
        try await OrganizationService().requireActiveSpaceCapacity(for: organizationID)
        for module in space.enabledModules {
            try await OrganizationService().requireModuleEntitlement(for: organizationID, module: module)
        }
        try await updateData([
            "organizationId": organizationID,
    ...
    func setArchived(_ isArchived: Bool, in space: Space) async throws {
        guard let firestore else { throw SpaceServiceError.firestoreNotConfigured }
        if !isArchived, let organizationID = space.organizationId {
            try await OrganizationService().requireActiveSpaceCapacity(for: organizationID)
        }
        try await updateData([
            "isArchived": isArchived,
            "archivedAt": isArchived ? FieldValue.serverTimestamp() : FieldValue.delete(),
            "updatedAt": FieldValue.serverTimestamp()
        ], for: firestore.collection("spaces").document(space.id))
    }
...
private final class CompositeSpaceListener: NSObject, ListenerRegistration {
    private var base: [ListenerRegistration] = []
    private var organization: [ListenerRegistration] = []
    func add(_ listener: ListenerRegistration) { base.append(listener) }
    func addOrganization(_ listener: ListenerRegistration) { organization.append(listener) }
    func removeOrganizationListeners() { organization.forEach { $0.remove() }; organization.removeAll() }
    func remove() { base.forEach { $0.remove() }; removeOrganizationListeners(); base.removeAll() }
}
