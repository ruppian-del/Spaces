...
final class ListService {
    ...
    func deleteList(_ list: SpaceList, in space: Space) async throws {
        ...
        for doc in items.documents {
            ...
            if let item = try? mapItem(doc, listID: list.id, key: key) {
                for attachment in item.attachments { try? await deleteTrackedStorage(path: attachment.storagePath, spaceID: space.id) }
            }
    ...
    func uploadAttachment(data: Data, name: String, mimeType: String, isMedia: Bool, space: Space, listID: String) async throws -> SpaceListItemAttachment {
        guard data.count <= 1_024 * 1_024 * 1_024 else { throw OrganizationServiceError.storageCapacityReached }
        let organizationID = try await OrganizationService().reserveStorage(forSpaceID: space.id, bytes: Int64(data.count))
        ...
        metadata.customMetadata = ["organizationChargedBytes": String(data.count)]
        do {
            try await storage.reference(withPath: path).putDataAsync(Data(base64Encoded: encrypted.ciphertext) ?? Data(), metadata: metadata)
        } catch {
            await OrganizationService().releaseStorage(organizationID: organizationID, bytes: Int64(data.count))
            throw error
        }
    ...
    private func deleteTrackedStorage(path: String, spaceID: String) async throws {
        let reference = storage.reference(withPath: path)
        let metadata = try? await reference.getMetadata()
        try await reference.delete()
        let charged = Int64(metadata?.customMetadata?["organizationChargedBytes"] ?? "") ?? 0
    ...
    private func mapList(_ doc: DocumentSnapshot, spaceID: String, key: SymmetricKey) throws -> SpaceList {
        ...
        return .init(
            ...
            sections: payload.sections.sorted { $0.order < $1.order }, links: payload.links,
    ...
    private func mapItem(_ doc: DocumentSnapshot, listID: String, key: SymmetricKey) throws -> SpaceListItem {
        ...
        return .init(
            ...
            order: data["order"] as? Int ?? 0, attachments: payload.attachments,
            links: payload.links ?? [],
            createdBy: data["createdBy"] as? String ?? "",
            createdAt: (data["createdAt"] as? Timestamp)?.dateValue() ?? Date(),
            updatedAt: (data["updatedAt"] as? Timestamp)?.dateValue() ?? Date()
