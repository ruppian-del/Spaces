...
struct SpaceSettingsView: View {
    ...
    private var organizationModuleIDs: Set<String>? {
        guard let organizationID = viewModel.space.organizationId else { return nil }
        return organizations.first { $0.id == organizationID }?.entitlements.enabledModuleIDs
    }

    private func moduleAvailable(_ module: SpaceModule) -> Bool {
        organizationModuleIDs?.contains(module.rawValue) ?? true
    }
            ...
            NavigationView {
                List(organizations) { organization in
                    Button(organization.name) {
                        Task { await viewModel.addToOrganization(organization.id) }
                        isShowingOrganizationPicker = false
                    }
                }
                .navigationTitle("Add to Organization")
            }
        }
        ...
        Section {
            if viewModel.canManageModules {
                ...
                Toggle("Announcements", isOn: announcementsEnabledBinding)
                    .disabled(viewModel.isUpdatingAnnouncementsModule || !moduleAvailable(.announcements))
                    .foregroundStyle(moduleAvailable(.announcements) ? .primary : .secondary)
                Toggle("Rooms", isOn: roomsEnabledBinding)
                    .disabled(viewModel.isUpdatingRoomsModule || !moduleAvailable(.rooms))
                    .foregroundStyle(moduleAvailable(.rooms) ? .primary : .secondary)
                Toggle("Lists", isOn: listsEnabledBinding)
                    .disabled(viewModel.isUpdatingListsModule || !moduleAvailable(.lists))
                    .foregroundStyle(moduleAvailable(.lists) ? .primary : .secondary)
                Toggle("Notes", isOn: notesEnabledBinding)
                    .disabled(viewModel.isUpdatingNotesModule || !moduleAvailable(.notes))
                    .foregroundStyle(moduleAvailable(.notes) ? .primary : .secondary)

                Toggle("Events", isOn: eventsEnabledBinding)
                    .disabled(viewModel.isUpdatingEventsModule || !moduleAvailable(.events))

                Toggle("Files", isOn: filesEnabledBinding)
                    .disabled(viewModel.isUpdatingFilesModule || !moduleAvailable(.files))
                    .foregroundStyle(moduleAvailable(.files) ? .primary : .secondary)

                Toggle("Polls", isOn: pollsEnabledBinding)
                    .disabled(viewModel.isUpdatingPollsModule || !moduleAvailable(.polls))
            } else {
                HStack {
                    Text("Announcements")
                    Spacer()
    ...
    private var organizationSection: some View {
        if viewModel.space.organizationId == nil {
            Section {
                Button("Add to Organization") { isShowingOrganizationPicker = true }
                    .disabled(organizations.isEmpty)
            } header: {
                Text("Organization")
            } footer: {
                Text(organizations.isEmpty ? "Set up an organization before attaching this Space." : "This keeps current members and the normal invite-only flow.")
            }
        }
    }
