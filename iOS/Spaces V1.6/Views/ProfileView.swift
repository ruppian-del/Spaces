...
struct ProfileView: View {
    ...
    @State private var isShowingFinalDeleteAccountConfirmation = false
        ...
        NavigationView {
            List {
                ...
                Section("Notifications") {
                    ...
                    NavigationLink(destination: SpaceNotificationsView()) {
                        Text("Space Notifications")
                    }
                ...
                Section {
                    Button(authViewModel.isDeletingAccount ? "Deleting Account..." : "Delete Account", role: .destructive) {
                        isShowingDeleteAccountConfirmation = true
                    }
                    .disabled(authViewModel.isDeletingAccount)
                } footer: {
                    Text("This permanently deletes your account, removes your profile and devices, and removes you from your Spaces.")
                }
            }
            ...
            .confirmationDialog(
                ...
                Button("Continue", role: .destructive) {
                    isShowingFinalDeleteAccountConfirmation = true
                }
                Button("Cancel", role: .cancel) { }
            } message: {
                Text("This starts permanent account deletion.")
            }
            .alert(
                "Permanently delete your account?",
                isPresented: $isShowingFinalDeleteAccountConfirmation
            ) {
                ...
                Text("This cannot be undone. Your account, profile, registered devices, and Space memberships will be permanently removed.")
            }
        }
...
private struct SpaceNotificationsView: View {
    @State private var spaces: [Space] = []
    @State private var settings: [String: SpaceNotificationPreference] = [:]
    @State private var spacesListener: ListenerRegistration?
    @State private var errorMessage: String?
    private let spaceService = SpaceService()
    private let preferences = SpaceNotificationPreferencesService()

    var body: some View {
        List {
            Section {
                Text("Choose a Space to customize which updates notify you. Activity remains available even when notifications are disabled.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
            }
            Section("Spaces") {
                if spaces.isEmpty {
                    SpaceEmptyStateView("No Spaces", systemImage: "rectangle.stack")
                }
                ForEach(spaces) { space in
                    NavigationLink {
                        SpaceNotificationDetailView(
                            space: space,
                            initialPreference: settings[space.id] ?? SpaceNotificationPreference()
                        ) { settings[space.id] = $0 }
                    } label: {
                        HStack(spacing: 12) {
                            Text(space.emoji).font(.title2)
                            Text(space.name)
                            Spacer()
                            Text((settings[space.id]?.allEnabled ?? true) ? "On" : "Off")
                                .foregroundStyle(.secondary)
                        }
                    }
                }
        ...
        .navigationTitle("Space Notifications")
        .navigationBarTitleDisplayMode(.inline)
        .onAppear {
            spacesListener = spaceService.listenToSpacesForCurrentUser {
                if case .success(let values) = $0 { spaces = values.sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending } }
            }
            Task {
                do { settings = try await preferences.load() }
                catch { errorMessage = error.localizedDescription }
            }
        }
        .onDisappear { spacesListener?.remove() }
        .alert("Space Notifications", isPresented: Binding(get: { errorMessage != nil }, set: { if !$0 { errorMessage = nil } })) {
            Button("OK") {}
...
private struct SpaceNotificationDetailView: View {
    let space: Space
    let initialPreference: SpaceNotificationPreference
    let onChange: (SpaceNotificationPreference) -> Void
    @State private var preference: SpaceNotificationPreference
    @State private var isSaving = false
    @State private var errorMessage: String?
    private let service = SpaceNotificationPreferencesService()

    private let moduleOptions = [
        (SpaceNotificationCategory.announcements, "Announcements", "megaphone.fill"),
        (SpaceNotificationCategory.rooms, "Room Messages", "bubble.left.and.text.bubble.right.fill"),
        (SpaceNotificationCategory.notes, "Notes", "note.text"),
        (SpaceNotificationCategory.lists, "Lists", "checklist"),
        (SpaceNotificationCategory.events, "Events", "calendar"),
        (SpaceNotificationCategory.polls, "Polls", "chart.bar.xaxis"),
        (SpaceNotificationCategory.mediaAndFiles, "Media & Files", "photo.on.rectangle")
    ]
    private let interactionOptions = [
        (SpaceNotificationCategory.commentsAndReplies, "Comments & Replies", "bubble.left"),
        (SpaceNotificationCategory.mentions, "Mentions", "at"),
        (SpaceNotificationCategory.assignments, "List Assignments", "person.crop.circle.badge.checkmark")
    ]

    init(
        space: Space,
        initialPreference: SpaceNotificationPreference,
        onChange: @escaping (SpaceNotificationPreference) -> Void
    ) {
        self.space = space
        self.initialPreference = initialPreference
        self.onChange = onChange
        _preference = State(initialValue: initialPreference)
    }
    ...
    var body: some View {
        List {
            Section {
                notificationToggle(
                    ...
                    isOn: Binding(
                        get: { preference.allEnabled },
                        set: { updateAll($0) }
                    )
                )
            } footer: {
                Text("Turning this off silences this Space without changing your individual choices.")
            }
            Section("Modules") {
                ForEach(moduleOptions, id: \.0) { option in
                    notificationToggle(
                        title: option.1,
                        systemImage: option.2,
                        isOn: categoryBinding(option.0)
                    )
                }
            }
            .disabled(!preference.allEnabled)
            Section("Interactions") {
                ForEach(interactionOptions, id: \.0) { option in
                    notificationToggle(
                        title: option.1,
                        systemImage: option.2,
                        isOn: categoryBinding(option.0)
        ...
        .navigationTitle("\(space.emoji) \(space.name)")
        .navigationBarTitleDisplayMode(.inline)
        .disabled(isSaving)
        .alert("Space Notifications", isPresented: Binding(get: { errorMessage != nil }, set: { if !$0 { errorMessage = nil } })) {
            Button("OK") {}
    ...
    private func notificationToggle(title: String, systemImage: String, isOn: Binding<Bool>) -> some View {
        Toggle(isOn: isOn) {
    ...
    private func categoryBinding(_ category: String) -> Binding<Bool> {
        Binding(
            get: { preference.categories[category] ?? true },
            set: { enabled in
                var updated = preference
                updated.categories[category] = enabled
                persist(updated)
            }
        )
    }

    private func updateAll(_ enabled: Bool) {
        var updated = preference
        updated.allEnabled = enabled
        persist(updated)
    }

    private func persist(_ updated: SpaceNotificationPreference) {
        ...
        Task {
            do {
                try await service.save(updated, spaceID: space.id)
            } catch {
                preference = previous
                onChange(previous)
