...
struct MessageBubbleView: View {
    ...
    @State private var isShowingReactionDetails = false
            ...
            HStack(spacing: 6) {
                ForEach(message.reactions) { reaction in
                    ...
                    .onLongPressGesture(minimumDuration: 0.4) {
                        isShowingReactionDetails = true
                    }
                    .accessibilityAddTraits(.isButton)
                    .accessibilityLabel("\(reaction.emoji) \(reaction.count) reactions")
                    .accessibilityHint("Double tap for no action. Press and hold to see who reacted.")
                }
            }
        }
        .frame(maxWidth: .infinity, alignment: alignment == .trailing ? .trailing : .leading)
        .alert("Reactions", isPresented: $isShowingReactionDetails) {
            Button("Done", role: .cancel) { }
        } message: {
            Text(reactionDetails.isEmpty ? "Reaction details are unavailable" : reactionDetails.joined(separator: "\n"))
        }
    }

    private var reactionDetails: [String] {
        message.reactions.flatMap { reaction in
            reaction.userNames.map { "\($0) reacted \(reaction.emoji)" }
        }
    }
