...
struct PollDetailView: View {
    ...
    private func voterName(for vote: SpacePollVote) -> String {
        viewModel.membersByID[vote.userID]?.displayName
            ?? vote.displayName?.trimmingCharacters(in: .whitespacesAndNewlines)
            ?? "Member"
    }

    private func voterEmoji(for vote: SpacePollVote) -> String {
        viewModel.membersByID[vote.userID]?.emojiAvatar ?? vote.emojiAvatar ?? "👤"
    }

    private func selectedOptions(for vote: SpacePollVote, in poll: SpacePoll) -> String {
        poll.options.filter { vote.optionIDs.contains($0.id) }.map(\.text).joined(separator: ", ")
    }
        ...
        Group {
            if let poll {
                List {
                    ...
                    if !poll.anonymous, !poll.votes.isEmpty {
                        Section("Who Voted") {
                            ForEach(poll.votes) { vote in
                                HStack(spacing: 12) {
                                    Text(voterEmoji(for: vote))
                                        .font(.title2)

                                    VStack(alignment: .leading, spacing: 3) {
                                        Text(voterName(for: vote))
                                            .font(.body.weight(.medium))
                                        Text(selectedOptions(for: vote, in: poll))
                                            .font(.caption)
                                            .foregroundStyle(.secondary)
                                    }
                                }
                    ...
                    Section("Results") {
                        ForEach(poll.options) { option in
                            PollOptionResultRow(
                                ...
                                isUpdating: viewModel.votingPollIDs.contains(poll.id)
                            ) {
                                Task {
                ...
                .navigationBarTitleDisplayMode(.inline)
                .toolbar {
                    if viewModel.canEdit(poll) || viewModel.canDelete(poll) {
                        ToolbarItem(placement: .navigationBarTrailing) {
                            Menu {
                                if viewModel.canEdit(poll) {
                                    Button("Edit Poll", systemImage: "pencil") { isPresentingEdit = true }
                                }
                                if viewModel.canDelete(poll) {
                                    Button("Delete Poll", systemImage: "trash", role: .destructive) { shouldConfirmDelete = true }
                                }
                            } label: {
                ...
                .sheet(isPresented: $isPresentingEdit) {
                    NavigationView {
                        CreatePollView(poll: poll) { question, options, closesAt, allowMultipleVotes, anonymous in
                            if await viewModel.updatePoll(poll, question: question, optionTexts: options, closesAt: closesAt, allowMultipleVotes: allowMultipleVotes, anonymous: anonymous) {
                                isPresentingEdit = false
                            }
                        } onCancel: { isPresentingEdit = false }
                    }
                }
...
private struct PollOptionResultRow: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .disabled(poll.isClosed || isUpdating)
    }
}
...
private struct CreatePollView: View {
    ...
    init(
        poll: SpacePoll? = nil,
        onCreate: @escaping (_ question: String, _ options: [String], _ closesAt: Date?, _ allowMultipleVotes: Bool, _ anonymous: Bool) async -> Void,
        onCancel: @escaping () -> Void
    ) {
        _question = State(initialValue: poll?.question ?? "")
        let options = poll?.options.map(\.text) ?? []
        _optionOne = State(initialValue: options.indices.contains(0) ? options[0] : "")
        _optionTwo = State(initialValue: options.indices.contains(1) ? options[1] : "")
        _optionThree = State(initialValue: options.indices.contains(2) ? options[2] : "")
        _optionFour = State(initialValue: options.indices.contains(3) ? options[3] : "")
        _hasClosingDate = State(initialValue: poll?.closesAt != nil)
        _closesAt = State(initialValue: poll?.closesAt ?? Date().addingTimeInterval(86_400))
        _allowMultipleVotes = State(initialValue: poll?.allowMultipleVotes ?? false)
        _anonymous = State(initialValue: poll?.anonymous ?? false)
        self.onCreate = onCreate
        self.onCancel = onCancel
        self.isEditing = poll != nil
    }
    ...
    var body: some View {
        ...
        .navigationTitle(isEditing ? "Edit Poll" : "Create Poll")
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            ...
            ToolbarItem(placement: .confirmationAction) {
                Button(isSubmitting ? "Saving..." : (isEditing ? "Save" : "Create")) {
                    isSubmitting = true
                    Task {
                        await onCreate(
                            question,
