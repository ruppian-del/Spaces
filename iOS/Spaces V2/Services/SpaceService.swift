...
final class SpaceService {
    ...
    func toggleReaction(
        ...
        let reference = firestore.collection("spaces")
            .document(space.id)
            .collection("messages")
            .document(messageID)
            .collection("reactions")
    ...
    func sendTextMessage(
        ...
        await recordActivity(
            ...
            title: replyContext == nil ? "sent a message" : "added a reply",
    ...
    func sendImageMessage(
        ...
        let messageReference = firestore.collection("spaces")
            ...
            .collection("messages")
            .document(mediaID)
        let mediaItems = try await withThrowingTaskGroup(of: (Int, SpaceMedia).self) { group in
            for (index, attachment) in imageAttachments.enumerated() {
                group.addTask { [encryptedMediaService] in
                    ...
                    let attachmentMediaID = "\(mediaID)-\(index)"
                    let uploadResult: EncryptedMediaUploadResult
                    if attachmentMediaType == .gif || attachmentMediaType == .meme {
                    ...
                    return (
                        ...
                        SpaceMedia(
                            ...
                            spaceID: space.id,
                            type: attachmentMediaType == .video ? .video : (attachmentMediaType == .gif ? .gif : (attachmentMediaType == .meme ? .meme : .image)),
            ...
            var uploadedItems: [(Int, SpaceMedia)] = []
            for try await result in group {
                uploadedItems.append(result)
            }
            return uploadedItems.sorted { $0.0 < $1.0 }.map(\.1)
        }

        guard let primaryMedia = mediaItems.first, let metadata = primaryMedia.metadata else {
            throw SpaceServiceError.invalidMediaData
        }
        ...
        messageData["mediaItems"] = mediaItems.enumerated().map { index, item in
            let itemMetadata = item.metadata!
            var payload: [String: Any] = [
                "id": item.id,
                "mediaId": itemMetadata.mediaId,
                "order": index,
                "mediaCategory": item.mediaCategory ?? itemMetadata.mediaType.rawValue,
                "mediaType": itemMetadata.mediaType.rawValue,
                "storagePath": itemMetadata.storagePath,
                "mediaStoragePath": itemMetadata.storagePath,
                "nonce": itemMetadata.nonce,
                "mediaNonceBase64": itemMetadata.nonce,
                "encryptionVersion": itemMetadata.encryptionVersion,
                "mimeType": itemMetadata.mimeType,
                "fileSize": itemMetadata.fileSize,
                "uploadedBy": itemMetadata.uploadedBy
            ]
            if let thumbnailStoragePath = itemMetadata.thumbnailStoragePath {
                payload["thumbnailStoragePath"] = thumbnailStoragePath
            }
            if let thumbnailNonce = itemMetadata.thumbnailNonce {
                payload["thumbnailNonce"] = thumbnailNonce
                payload["thumbnailNonceBase64"] = thumbnailNonce
            }
            if let width = itemMetadata.width {
                payload["width"] = width
            }
            if let height = itemMetadata.height {
                payload["height"] = height
            }
            if let duration = itemMetadata.duration {
        ...
        await recordActivity(
            ...
            actorName: senderName,
            actorEmoji: senderEmoji,
            title: imageAttachments.count > 1
                ? "shared photos"
                : (resolvedMediaType == .gif ? "shared a GIF" : (resolvedMediaType == .meme ? "shared a meme" : "shared a photo")),
    ...
    private func currentUserHasPermission(_ permission: SpacePermission, in space: Space) async throws -> Bool {
        ...
        return try await fetchCurrentUserRole(in: space).capabilities.contains(permission)
    }
