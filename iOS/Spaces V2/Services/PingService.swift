...
final class PingService {
    ...
    func sendImageMessage(
        ...
        guard !imageAttachments.isEmpty else {
            throw PingServiceError.invalidMediaData
        }
        ...
        let messageReference = firestore.collection("pings")
            ...
            .collection("messages")
            .document(mediaID)
        let uploadedMedia: [(order: Int, metadata: EncryptedMediaMetadata)] = try await withThrowingTaskGroup(
            of: (Int, EncryptedMediaMetadata).self
        ) { group in
            for (index, attachment) in imageAttachments.enumerated() {
                group.addTask { [encryptedMediaService] in
                    let attachmentType = MediaType(rawValue: attachment.mediaCategory) ?? .photo
                    let uploadResult: EncryptedMediaUploadResult
                    if attachmentType == .gif || attachmentType == .meme {
                    ...
                    return (index, uploadResult.metadata)
                }
            }

            var results: [(Int, EncryptedMediaMetadata)] = []
            for try await result in group {
                results.append(result)
            }
            return results.sorted { $0.0 < $1.0 }.map { (order: $0.0, metadata: $0.1) }
        }

        guard let primaryMetadata = uploadedMedia.first?.metadata else {
            throw PingServiceError.invalidMediaData
        }

        print("[PingService][ImageMessage] mediaSelected=true attachmentCount=\(imageAttachments.count) mediaCategory=\(firstAttachment.mediaCategory)")
        for item in uploadedMedia {
            let attachment = imageAttachments[item.order]
            print("[PingService][ImageMessage] uploadPath=\(item.metadata.storagePath) byteCount=\(attachment.data.count) thumbnailByteCount=\(attachment.previewImageData.count)")
            if let thumbnailStoragePath = item.metadata.thumbnailStoragePath {
        ...
        let mediaItemsPayload: [[String: Any]] = uploadedMedia.enumerated().map { index, item in
            ...
            var payload: [String: Any] = [
                ...
                "mediaType": item.metadata.mediaType.rawValue,
                "storagePath": item.metadata.storagePath,
                "mediaStoragePath": item.metadata.storagePath,
                "nonce": item.metadata.nonce,
                "mediaNonceBase64": item.metadata.nonce,
                "encryptionVersion": item.metadata.encryptionVersion,
            ...
            if let thumbnailNonce = item.metadata.thumbnailNonce {
                payload["thumbnailNonce"] = thumbnailNonce
                payload["thumbnailNonceBase64"] = thumbnailNonce
            }
            if let width = item.metadata.width {
                payload["width"] = width
            }
            if let height = item.metadata.height {
        ...
        var messageData: [String: Any] = [
            "id": mediaID,
            "mediaId": primaryMetadata.mediaId,
            "pingId": ping.id,
            "senderId": session.uid,
            "senderName": senderName,
            "senderEmoji": senderEmoji,
            ...
            "mediaType": primaryMetadata.mediaType.rawValue,
            "storagePath": primaryMetadata.storagePath,
            "nonce": primaryMetadata.nonce,
            "mediaStoragePath": primaryMetadata.storagePath,
            "mediaNonceBase64": primaryMetadata.nonce,
            "encryptionVersion": primaryMetadata.encryptionVersion,
            "mimeType": primaryMetadata.mimeType,
            "fileSize": primaryMetadata.fileSize,
            "uploadedBy": primaryMetadata.uploadedBy,
        ...
        if let thumbnailStoragePath = primaryMetadata.thumbnailStoragePath {
            messageData["thumbnailStoragePath"] = thumbnailStoragePath
        }
        if let thumbnailNonce = primaryMetadata.thumbnailNonce {
            messageData["thumbnailNonce"] = thumbnailNonce
            messageData["thumbnailNonceBase64"] = thumbnailNonce
        }
        if let width = primaryMetadata.width {
            messageData["width"] = width
        }
        if let height = primaryMetadata.height {
        ...
        let mediaItems = uploadedMedia.enumerated().map { index, item in
            let attachment = imageAttachments[index]
            let attachmentType = MediaType(rawValue: attachment.mediaCategory) ?? .photo
            ...
            return SpaceMedia(
                ...
                placeholderImageName: item.metadata.mediaType.defaultPlaceholderImageName,
                ...
                mediaNonceBase64: item.metadata.nonce,
                thumbnailNonceBase64: item.metadata.thumbnailNonce
            )
        }
    ...
    private func mapMessage(
        ...
        if [.image, .video, .meme, .gif, .screenshot].contains(type) {
            ...
            let mediaItems = mappedMediaItems(
                ...
                pingID: pingIdentifier.isEmpty ? nil : pingIdentifier,
                senderID: senderID,
                senderName: senderName,
                caption: caption,
                createdAt: createdAt,
            ...
            if type == .gif || primaryMedia?.mediaCategory == "gif" || primaryMedia?.mediaType == .gif {
                print("[GIF Receive] message model mapped id=\(document.documentID) itemCount=\(mediaItems.count) primaryType=\(primaryMedia?.type.rawValue ?? "") storagePath=\(primaryMedia?.metadata?.storagePath ?? "")")
            }
    ...
    private func mappedMediaItems(
        ...
        return itemDictionaries.compactMap { item in
            ...
            let metadata = EncryptedMediaMetadata(
                ...
                encryptionVersion: inferredEncryptionVersion(from: item),
...
enum PingServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidMediaData:
            return "That media could not be prepared for sending."
        case .messageNotFound:
