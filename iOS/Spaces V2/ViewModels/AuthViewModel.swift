...
final class AuthViewModel: ObservableObject {
    ...
    func deleteAccount() {
        ...
        guard currentSession != nil else {
            activeAlert = AuthAlert(title: "Delete Account", message: "Sign in before deleting your account.")
            return
        }
        ...
        Task {
            ...
            do {
                let message = try await authService.deleteCurrentAccount()
                isAuthenticated = false
                requiresProfileCreation = false
                currentSession = nil
                currentUserProfile = nil
                activeAlert = AuthAlert(title: "Account Deleted", message: message)
            } catch {
                activeAlert = AuthAlert(title: "Delete Account", message: error.localizedDescription)
            }
        }
    }
