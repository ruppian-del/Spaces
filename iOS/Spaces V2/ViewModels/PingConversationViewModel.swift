...
final class PingConversationViewModel: ObservableObject {
    ...
    func sendComposer() async {
        ...
        } else if let selectedComposerMedia = selectedComposerMediaItems.first, !selectedComposerMediaItems.isEmpty {
    ...
    func removeComposerMedia(id: UUID? = nil) {
        ...
        print("[PingConversationViewModel][ComposerMedia] selected=\(!selectedComposerMediaItems.isEmpty) count=\(selectedComposerMediaItems.count) removed=true")
    }
    ...
    private func sendImages(_ selections: [ComposerMediaSelection], caption: String?) async {
        guard !isSending else { return }
        ...
        do {
            let localMessage = try await pingService.sendImageMessage(
                in: ping,
                imageAttachments: selections.map {
                    PingService.ImageAttachmentUpload(
                        data: $0.data,
                        previewImageData: $0.previewImageData,
                        mimeType: $0.mimeType,
                        mediaCategory: $0.mediaCategory
                    )
                },
                caption: caption,
                replyContext: activeReplyContext()
            ...
            pendingLocalMessagesByID[localMessage.id] = localMessage
            refreshMessages(baseMessages)
            if selections.first?.mediaCategory == "gif" {
