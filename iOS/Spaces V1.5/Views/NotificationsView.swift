...
struct NotificationsView: View {
    ...
    private func destinationView(for item: SpaceNotificationItem) -> some View {
        if let space = viewModel.spacesByID[item.spaceId] {
            ...
            case .announcements:
                AnnouncementsView(space: space, initialAnnouncementID: item.targetId)
            case .rooms:
                RoomsView(space: space, initialRoomID: item.targetId)
            case .lists:
                ListsView(space: space, initialListID: item.targetId)
            case .notes:
                NotesView(space: space, initialNoteID: item.targetId)
            case .space, .none:
