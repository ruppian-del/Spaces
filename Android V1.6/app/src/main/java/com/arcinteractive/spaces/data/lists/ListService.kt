...
class ListService {
    ...
    suspend fun deleteItem(context: Context, space: Space, list: SpaceList, item: SpaceListItem) {
        ...
        item.attachments.forEach { runCatching { deleteTrackedStorage(context, space.id, it.storagePath) } }
    ...
    suspend fun deleteList(context: Context, space: Space, list: SpaceList) {
        ...
        items.documents.forEach { document ->
            runCatching { mapItem(document, list.id, key) }.getOrNull()?.attachments?.forEach {
                runCatching { deleteTrackedStorage(context, space.id, it.storagePath) }
            }
    ...
    private suspend fun awaitUpload(path: String, bytes: ByteArray, chargedBytes: Long) = kotlinx.coroutines.suspendCancellableCoroutine<Unit> { c ->
    ...
    private suspend fun deleteTrackedStorage(context: Context, spaceId: String, path: String) {
        ...
        val charged = kotlinx.coroutines.suspendCancellableCoroutine<Long> { c -> reference.metadata
