...
class RoomService {
    ...
    fun deleteRoom(context: android.content.Context, room: SpaceRoom, onComplete: (Result<Unit>) -> Unit) {
        ...
        roomReference.collection("messages").get()
            .addOnSuccessListener { snapshot ->
                CoroutineScope(Dispatchers.IO).launch {
                    snapshot.documents.flatMap { document -> (document.get("attachments") as? List<*>).orEmpty() }
                        .mapNotNull { (it as? Map<*, *>)?.get("storagePath") as? String }
                        .forEach { path ->
                            val reference = FirebaseStorage.getInstance().reference.child(path)
                            val charged = kotlinx.coroutines.suspendCancellableCoroutine<Long> { continuation -> reference.metadata
                                .addOnSuccessListener { continuation.resume(it.getCustomMetadata("organizationChargedBytes")?.toLongOrNull() ?: 0L) {} }
                                .addOnFailureListener { continuation.resume(0L) {} } }
                            runCatching { kotlinx.coroutines.suspendCancellableCoroutine<Unit> { continuation -> reference.delete()
                                .addOnSuccessListener { continuation.resume(Unit) {} }.addOnFailureListener(continuation::resumeWithException) } }
                            com.arcinteractive.spaces.data.organization.OrganizationService().releaseStorageForSpace(context, room.spaceId, charged)
                        }
    ...
    fun deleteMessage(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            message.attachments.forEach { attachment ->
                val reference = FirebaseStorage.getInstance().reference.child(attachment.storagePath)
                val charged = kotlinx.coroutines.suspendCancellableCoroutine<Long> { continuation -> reference.metadata
                    .addOnSuccessListener { continuation.resume(it.getCustomMetadata("organizationChargedBytes")?.toLongOrNull() ?: 0L) {} }
                    .addOnFailureListener { continuation.resume(0L) {} } }
                runCatching { kotlinx.coroutines.suspendCancellableCoroutine<Unit> { continuation -> reference.delete()
    ...
    fun sendAttachment(
        ...
        if (bytes.size.toLong() > 1024L * 1024L * 1024L) return onComplete(Result.failure(IllegalArgumentException("Each upload is limited to 1 GB.")))
        val userId = FirebaseAuth.getInstance().currentUser?.uid
            ?: return onComplete(Result.failure(IllegalStateException("You must be signed in.")))
        requirePostingPermission(spaceId, roomId, userId) { permissionResult ->
            permissionResult.onFailure {
            ...
            ensureRoomKey(spaceId, roomId) { keyResult ->
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    val organizationId = runCatching { organizations.reserveStorage(context, spaceId, bytes.size.toLong()) }
                    ...
                    val reference = FirebaseStorage.getInstance().reference.child(path)
                    val metadata = com.google.firebase.storage.StorageMetadata.Builder().setCustomMetadata("organizationChargedBytes", bytes.size.toString()).build()
                    reference.putBytes(Base64.decode(encrypted.ciphertext, Base64.NO_WRAP), metadata)
                    .addOnFailureListener { error -> CoroutineScope(Dispatchers.IO).launch { organizations.releaseStorage(context, organizationId, bytes.size.toLong()); onComplete(Result.failure(error)) } }
                    .addOnSuccessListener {
                        ...
                        firestore.collection("spaces").document(spaceId)
                            ...
                            .addOnSuccessListener { onComplete(Result.success(Unit)) }
                            .addOnFailureListener { error -> CoroutineScope(Dispatchers.IO).launch {
                                runCatching { kotlinx.coroutines.suspendCancellableCoroutine<Unit> { continuation -> reference.delete().addOnSuccessListener { continuation.resume(Unit) {} }.addOnFailureListener(continuation::resumeWithException) } }
