...
private fun RoomConversationScreen(
        ...
        if (captured && file != null) {
            runCatching {
                roomService.sendAttachment(
                    context, space.id, room.id, senderName, file.readBytes(), file.name, "image/jpeg", true
    ...
    val mediaPicker = rememberLauncherForActivityResult(ActivityResultContracts.PickVisualMedia()) { uri ->
        ...
        runCatching {
            ...
            roomService.sendAttachment(context, space.id, room.id, senderName, bytes, "Shared Media", mime, true) {
    ...
    val filePicker = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        runCatching {
            ...
            roomService.sendAttachment(context, space.id, room.id, senderName, bytes, name, mime, false) {
    ...
    deletingMessage?.let { message ->
        AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = {
                    roomService.deleteMessage(context, space.id, room.id, message, canDeleteOthersMessages) { result ->
    ...
    if (isEditingRoom) {
        EditRoomDialog(
            ...
            onDelete = {
                roomService.deleteRoom(context, room) { result ->
