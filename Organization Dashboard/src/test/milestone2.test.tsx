...
describe('Milestone 2 onboarding states', () => {
  ...
  it('shows validation, all major steps, and the exact Foundation summary', () => {
    render(<MemoryRouter><OrganizationOnboardingPage /></MemoryRouter>)
    expect(screen.getByLabelText('Organization name')).toHaveFocus()
    expect(screen.getByRole('list', { name: 'Organization setup progress' }).querySelector('[aria-current="step"]')).toHaveTextContent('Identity')
    ...
    fireEvent.click(screen.getByRole('button', { name: /continue/i }))
    expect(screen.getByRole('heading', { name: 'Foundation includes' })).toBeInTheDocument()
    expect(screen.getByText('250')).toBeInTheDocument(); expect(screen.getByText('10 GB')).toBeInTheDocument()
    expect(screen.getByText('Space Pings')).toBeInTheDocument(); expect(screen.queryByText('Announcements')).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /continue/i }))
  ...
  it('prevents repeat clicks while creating and renders success', async () => {
    ...
    const submit = screen.getByRole('button', { name: 'Create Foundation organization' })
  ...
  it('renders a retry-safe network failure', async () => {
    ...
    render(<MemoryRouter><OrganizationOnboardingPage /></MemoryRouter>)
    fireEvent.change(screen.getByLabelText('Organization name'), { target: { value: 'ArcInteractive' } })
    fireEvent.click(screen.getByRole('button', { name: /continue/i })); fireEvent.click(screen.getByRole('button', { name: /continue/i })); fireEvent.click(screen.getByRole('button', { name: /continue/i })); fireEvent.click(screen.getByRole('button', { name: 'Create Foundation organization' }))
