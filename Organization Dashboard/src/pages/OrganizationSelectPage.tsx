...
export function OrganizationSelectPage() {
  ...
  return <div className="app-page"><header className="topbar"><Brand compact /><div className="topbar-actions"><button className="icon-button" aria-label="Sign out" onClick={() => void signOutUser()}><LogOut size={19} /></button></div></header><main className="selection"><div className="selection-heading"><div><span className="eyebrow">Organization dashboard</span><h1>Choose an organization</h1><p>Select an existing organization or set up a new Foundation organization.</p></div><button className="primary-button" onClick={() => navigate('/organizations/new')}><Plus size={18} /> Create organization</button></div>{state.kind === 'loading' || state.kind === 'error' || state.kind === 'permission-denied' ? <StateView state={state} onRetry={state.kind === 'error' ? load : undefined} /> : organizations.length ? <div className="org-grid">{organizations.map((org) => <button className="org-option" key={org.id} onClick={() => navigate(`/organizations/${org.id}`, { state: { organization: org } })}><span className="org-avatar"><Building2 /></span><span><strong>{org.name}</strong><small>{org.status === 'active' ? 'Active organization' : 'Suspended organization'}</small></span><ChevronRight /></button>)}</div> : <div className="selection-empty"><Building2 /><h2>No organizations yet</h2><p>Create your first Foundation organization to begin.</p></div>}</main></div>
}
