...
import { Archive, Building2, CreditCard, Database, ExternalLink, LayoutGrid, LogOut, Puzzle, Settings, ShieldCheck, Users } from 'lucide-react'
...
export function DashboardContent({ data }: { data: DashboardData }) {
  ...
  return <><div className="dashboard-heading"><div><span className="eyebrow">Organization overview</span><h1>{org.name}</h1>{org.description && <p>{org.description}</p>}<div className="status-line"><span className={`status-dot ${org.status}`} /> {org.status === 'active' ? 'Active' : 'Suspended'}</div></div><div className="dashboard-actions"><Link className="secondary-button" to="/spaces"><LayoutGrid size={17} /> Space Dashboards</Link><Link className="secondary-button" to={`/organizations/${org.id}/billing`}><CreditCard size={17} /> Plan</Link><Link className="secondary-button" to={`/organizations/${org.id}/settings`}><Settings size={17} /> Settings</Link></div></div>
